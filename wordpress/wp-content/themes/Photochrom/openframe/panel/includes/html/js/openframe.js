...
jQuery( document ).ready( function( $ ) {
	...
	op_panel_tabs.each( function() {
		...
		op_panel_menu.append( "<a href=\"#\" class=\"nav-tab\">" + $( this ).data( "tab" ) + "</a>");
	...
	function op_panel_tab_switch( name ) {
		...
		tab = op_panel_tabs.filter( "[data-tab=" + name + "]" )
										
		tab.addClass("active").siblings(".op-panel-section").removeClass("active")
				
		op_panel_menu.find( ":contains('" + name + "')" ).addClass("nav-tab-active").siblings().removeClass("nav-tab-active")
		
		op_panel_tab_put( name )
				
	}
		
	op_panel_tab_switch( op_panel_tab_get() ? op_panel_tab_get() : op_panel_tabs.eq( 0 ).data("tab") )
	...
	.on( "click", function( e ) {
		...
		container.find("input:hidden").val( null )
